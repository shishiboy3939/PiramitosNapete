...
half3 t2d(float2 p){
   half3 col = SAMPLE_TEXTURE2D(_BlitTexture, sampler_PointClamp, p).xyz; 
   return rgb2yiq( col );
}
...
half3 bleed( Varyings input ){
   ...
   half3 signal = half3(0,0,0);
      ...
      half3 sums =  half3(0,0,0);
      float blur = 0.; // always extra blur
      // float blur = -.5; // extra blur

      int taps = bleedLength-4;
      for (int j = 0; j < taps % 1023; j++){

         //v4.1
         float off = float(j) - float(taps);

         if(bleedMode==0){
            sums =   t2d( float2(p.x + (+off + blur) *_One.x *bleedAmount, p.y) );
         }else if(bleedMode==1){
            sums =   t2d( float2(p.x + (-off + blur) *_One.x *bleedAmount, p.y) ) ;
         }else{
            sums =   t2d( float2(p.x + (+off + blur) *_One.x *bleedAmount, p.y) ) +
                     t2d( float2(p.x + (-off + blur) *_One.x *bleedAmount, p.y) ) ;
         }

         adj = half3(LF[j+3], CF[j], CF[j]);
         signal += sums * adj;
         
         if(bleedMode==0 || bleedMode==1){
            norm += adj;
         }else{
            norm += adj*2.;            
         }
      ...
      signal += t2d( float2(p.x + blur *_One.x *bleedAmount, p.y) ) * adj;
      norm += adj;
      signal = signal / norm;
   ...
   if(feedbackOn==1){
      half3 fbb = SAMPLE_TEXTURE2D(_TexFeedback, sampler_PointClamp, p).xyz; 
      // half3 fbb = SAMPLE_TEXTURE2D_X(_TexFeedback, sampler_TexFeedback, p ).xyz; //output color
      col = bms(col, fbb*1.);       
   }
