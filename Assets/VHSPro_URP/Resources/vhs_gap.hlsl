...
half4 getPal(float pos){
   return SAMPLE_TEXTURE2D(_TexPalette, sampler_PointClamp, float2(pos/_ResPalette,.5) ).xyzw; //need uint coz of lerp
}
...
half3 palette(half3 col){
   ...
   while(step>=1.){   
      half4 cs = getPal(pos);
      pc = cs.a;//tex2D( _TexPalette, float2(pos * ox, 0.5)).a;                          
      step /= 2.;
   ...
   half3 cc = getPal(pos).rgb;// tex2D( _TexPalette, float2(pos * ox, .5)).rgb;      
   float l = len(cc, mcol);
   float nl = 0.;// new len
   float pp = 0.; //TODO int
   ...
   float ii = -paletteDelta; while(ii<paletteDelta){ ii += 1.0;
      ...
      if( pp>=0. && pp<float(_ResPalette) ){
         cc = getPal(pp).rgb;//tex2D( _TexPalette, float2( pp * ox, .5)).rgb;
         nl = len(cc, mcol);
         if(nl<l) {
            l = nl;//wasnt here
            bestPos = pp;
   ...
   col.xyz = getPal(bestPos).rgb;//tex2D( _TexPalette, float2(bestPos * ox, .5)).rgb;
   return col;
}
   ...
   //minX - black point
   half whiteBlackPoint(half x, half minX, half maxX){

      // black point
      x = max(x, minX) * lerp(1., 1./(1.-minX), signalNorm);

      //white point 
      x = min(x, maxX) * lerp(1., 1./maxX, signalNorm);

      return x;
   } 

   //limiting color value range for chroma channels. where values are -1..0..1
   half whiteBlackPointChrom(half x, half minX, half maxX){

      // black point
      x = max(x, minX) * lerp(1., ((x<0.)? 1./(-minX) : 1.), signalNorm);

      //white point 
      x = min(x, maxX) * lerp(1., ((x>0.)? 1./maxX : 1.), signalNorm);
   ...
   half3 signalTweak(half3 signal){
      ...
      //black white point filter
      signal.x = whiteBlackPoint(signal.x, signalMin.x, signalMax.x);
      signal.y = whiteBlackPointChrom(signal.y, signalMin.y, signalMax.y);
      signal.z = whiteBlackPointChrom(signal.z, signalMin.z, signalMax.z);
