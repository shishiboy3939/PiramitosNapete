...
//film grain, line noise, tape noise
float tapeNoiseAlpha;
...
half3 yiq2rgb(half3 c){       
   ...
   float3 jitterV(float2 p, float m, float t){
      ...
      half3 signal = half3(
         rgb2yiq( SAMPLE_TEXTURE2D(_BlitTexture, sampler_PointClamp, float2(offx.r, p.y) ).rgb ).x,
         rgb2yiq( SAMPLE_TEXTURE2D(_BlitTexture, sampler_PointClamp, float2(offx.g, p.y) ).rgb ).y,
         rgb2yiq( SAMPLE_TEXTURE2D(_BlitTexture, sampler_PointClamp, float2(offx.b, p.y) ).rgb ).z
      );
   ...
   float2 tapeNoiseUV(float2 p, float2 pn){
      ...
      for(int ii = 0; ii < distWidth % 1023; ii++){
        ...
        float tnl = SAMPLE_TEXTURE2D(_TexTape, sampler_PointClamp, float2(0., pn.y-_OneN.x*ii) ).y;

        // float tnl = tex2Dlod(_TexTape, float4(0., pn.y-_OneN.x*ii, 0., 0.)).y;
        // float tnl = tex2Dlod(_TexTape, float4(0.0,pn.y-ONEXN*ii, 0.0, 0.0)).y;
        // float tnl = tex2D(_TexTape, float2(0.0,pn.y-ONEXN*ii)).y;
        // float tnl = tapeNoiseLines(float2(0.0,pn.y-ONEXN*i), t*tapeNoiseSpeed)*tapeNoiseAlpha;
   ...
   half3 tapeNoiseCol(half3 signal, half4 colN, float2 pn){
      ...
      half tn = colN.x; //v2.0
      // half tn = tex2D(_TexTape, pn).x;
      signal.x = bms(signal.x, tn * tapeNoiseAlpha);  
      // float tn = tapeNoise(pn, t*tapeNoiseSpeed)*tapeNoiseAlpha;
      ...
      for(int j=0; j<tailLength % 1023; j++){
         ...
         float4 colN2 = SAMPLE_TEXTURE2D(_TexTape, sampler_PointClamp, d).xyzw; 

         tn = colN2.x; //v2.0
         // tn = tex2Dlod(_TexTape, float4(d,0,0) ).x;
         // float2 d = float2(pn.x-ONEXN*jj,pn.y);
         // tn = tex2D(_TexTape, d).x;
         // tn = tapeNoise(float2(pn.x-ONEXN*i,pn.y), t*tapeNoiseSpeed)*tapeNoiseAlpha;

         //for tails length difference
         float fadediff = colN2.a; //v2.0
         // float fadediff = tex2D(_TexTape, d).a; //hash12(d); 
         ...
         if( tn > .8 ){               
            ...
            signal.x = bms(signal.x, nsx * tapeNoiseAlpha);                  
         }
      ...
      if(distShift>.4){
         // float tnl = tapeNoiseLines(float2(0.0,pn.y), t*tapeNoiseSpeed)*tapeNoiseAlpha;
         float tnl = colN.y;//v2.0
         // float tnl = tex2D(_TexTape, pn).y;//tapeNoiseLines(float2(0.0,pn.y), t*tapeNoiseSpeed)*tapeNoiseAlpha;
...
half3 vhs( Varyings input ){
   ...
   // pn.y = floor( pn.y*_ResN.y )/_ResN.y;
   // pn = floor(po*_ResN.xy) / _ResN.xy;
   ...
   half3 signal = half3(0,0,0);  //signal has negative values
      ...
      col = SAMPLE_TEXTURE2D(_BlitTexture, sampler_PointClamp, p).xyz; 
      signal = rgb2yiq(col);
   ...
   #if VHS_TAPENOISE_ON || VHS_LINENOISE_ON || VHS_FILMGRAIN_ON 
      colN = SAMPLE_TEXTURE2D(_TexTape, sampler_PointClamp, pn).xyzw;      
