...
namespace UnityEditor.Rendering {
	...
	[CustomEditor(typeof(VHSProVolumeComponent))]
	class VHSProEditor : VolumeComponentEditor {
		...
		SerializedDataParameter bleedMode;
		...
		SerializedDataParameter tapeNoiseAmt; 
		SerializedDataParameter tapeNoiseAlpha; 
		...
		SerializedDataParameter signalRangeY; 
		SerializedDataParameter signalRangeI; 
		SerializedDataParameter signalRangeQ; 
		SerializedDataParameter signalNorm; 
		...
		public override void OnEnable() {
			...
			var o = new PropertyFetcher<VHSProVolumeComponent>(serializedObject);
			...
			bleedMode = 			Unpack(o.Find("bleedMode"));
			...
			tapeNoiseAmt = 			Unpack(o.Find("tapeNoiseAmt")); 
			tapeNoiseAlpha = 	Unpack(o.Find("tapeNoiseAlpha")); 
			...
			signalRangeY = 		Unpack(o.Find("signalRangeY")); 
			signalRangeI = 		Unpack(o.Find("signalRangeI")); 
			signalRangeQ = 		Unpack(o.Find("signalRangeQ")); 
			signalNorm = 			Unpack(o.Find("signalNorm")); 
		...
		public override void OnInspectorGUI() {
			...
			if(g_color.boolValue){
		      ...
		      //SIGNAL
		   	PropertyField(signalTweakOn, EditorGUIUtility.TrTextContent("Signal Tweak", ""));
					...
					EditorGUILayout.LabelField("     Black/White Point Filter");

					Vector2 signalRangeY_prev = signalRangeY.value.vector2Value; 
					PropertyField(signalRangeY, EditorGUIUtility.TrTextContent("Black/White Y", ""));
					PropertyField(signalRangeI, EditorGUIUtility.TrTextContent("Blue/Orange I", ""));
					PropertyField(signalRangeQ, EditorGUIUtility.TrTextContent("Green/Purple Q", ""));

					//check that min <= max
					if(signalRangeY.value.vector2Value.x != signalRangeY_prev.x && 
						signalRangeY.value.vector2Value.x > signalRangeY.value.vector2Value.y){
						signalRangeY.value.vector2Value = new Vector2(signalRangeY.value.vector2Value.y, signalRangeY.value.vector2Value.y);
					}
					if(signalRangeY.value.vector2Value.y != signalRangeY_prev.y && 
						signalRangeY.value.vector2Value.y < signalRangeY.value.vector2Value.x){
						signalRangeY.value.vector2Value = new Vector2(signalRangeY.value.vector2Value.x, signalRangeY.value.vector2Value.x);
					}

					//clamp range limits 
					signalRangeY.value.vector2Value = VHSHelper.Vector2Clamp(signalRangeY.value.vector2Value, Vector2.zero, Vector2.one);
					signalRangeI.value.vector2Value = new Vector2(Mathf.Clamp(signalRangeI.value.vector2Value.x, -1f, 0f), Mathf.Clamp(signalRangeI.value.vector2Value.y, 0f, 1f));
					signalRangeQ.value.vector2Value = new Vector2(Mathf.Clamp(signalRangeQ.value.vector2Value.x, -1f, 0f), Mathf.Clamp(signalRangeQ.value.vector2Value.y, 0f, 1f));

					PropertyField(signalNorm, EditorGUIUtility.TrTextContent("Normalization", ""));
	      ...
	      //CRT
		   ...
		   if(g_crt.boolValue){
            ...
            using (new EditorGUILayout.HorizontalScope()) { DrawOverrideCheckbox(bleedMode);
       		using (new EditorGUI.DisabledScope(!bleedMode.overrideState.boolValue)) {

				   bleedMode.value.intValue = EditorGUILayout.Popup("Bleed Mode", bleedMode.value.intValue,  
				   		new string[3] {"Left", "Right", "Both"}); 	
				}}
		   ...
		   if(g_noise.boolValue){		   
      		...
      		PropertyField(noiseResGlobal, EditorGUIUtility.TrTextContent("Global Resolution", ""));
			   ...
			   PropertyField(tapeNoiseOn, EditorGUIUtility.TrTextContent("Tape Noise", ""));	
					indP();
					PropertyField(tapeNoiseAmt, EditorGUIUtility.TrTextContent("Amount", ""));
					PropertyField(tapeNoiseSpeed, EditorGUIUtility.TrTextContent("Speed", ""));
					PropertyField(tapeNoiseAlpha, EditorGUIUtility.TrTextContent("Alpha", ""));
				   indM();
					EditorGUILayout.Space();	   
			}
